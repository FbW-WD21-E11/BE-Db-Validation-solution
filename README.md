...
## Task 5 - Fixing our schema to our collection

Since we have already created a database and collection, we already know where our data is going

We want to make sure that Mongoose (and MongoDB) use the correct collection

One way we can do this is to directly provide our Schema with the name of our collection.

1. In the constructor where you define your schema, add the following object as a second argument:

```javascript
{ collection: "pokedex" }
```

This will force MongoDB to use the exact name **_pokedex_** when referring to the collection
...
## Task 7 - Schema validation with enum
...
## Task 10 - Creating an endpoint - get all pokemon
...
## Task 11 - Creating an endpoint - Create a new pokemon
...
## Task 13 - Creating an endpoint - search for pokemon by name
...
## Task 14 - Creating an endpoint - search for pokemon by id
...
## Task 15 - Creating an endpoint - search for pokemon by type
...
## Task 16 - Modifying an endpoint - search for pokemon by name and language
